...
class User_Learn_HistoryService{
    async createStack(user_learn_historyData){
        const user_learn_history = await User_Learn_History.create({...user_learn_historyData})

        return user_learn_history;
    }

    async getAll(id_learn){
        const user_learns = await User_Learn_History.findAll({where: {id_learn}, attributes: { exclude: ['createdAt', 'updatedAt'] } });
